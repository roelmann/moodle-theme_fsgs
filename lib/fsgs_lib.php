...
function theme_fsgs_get_course_activities() {
...
function theme_fsgs_strip_html_tags( $text ) {
    $text = preg_replace(
        array(
 ...
 * @return string
 */
function theme_fsgs_course_trim_char($str, $n = 500, $endchar = '&#8230;') {
    if (strlen($str) < $n) {
        return $str;
    }
 ...
 */
function fsgs_boostnavigation_find_privatefiles_node(global_navigation $navigation) {
 ...
 * @return array
 */
function fsgs_boostnavigation_get_all_childrenkeys(navigation_node $navigationnode) {
    ...
    // Yes, this node has children.
        ...
        foreach ($childrennodeskeys as $ck) {
            $allchildren = array_merge($allchildren, fsgs_boostnavigation_get_all_childrenkeys($navigationnode->get($ck)));
        }
 ...
 * @return void
 */
function fsgs_local_navigation_extend_navigation(global_navigation $navigation) {
    global $PAGE;

        $menu = new custom_menu($PAGE->theme->settings->adddrawermenu, current_language());
        if ($menu->has_children()) {
            foreach ($menu->get_children() as $item) {
                fsgs_navigation_custom_menu_item($item, 0, null);
            }
        }
}
/**
