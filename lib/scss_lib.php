 ...
 */
function theme_fsgs_css_tree_post_processor($tree, $theme) {
 ...
 * @return string
 */
function theme_fsgs_get_main_scss_content($theme) {
    global $CFG;
    ...
    $iterator = new DirectoryIterator($CFG->dirroot . '/theme/fsgs/scss/preset/');
    $presetisset = '';
    foreach ($iterator as $pfile) {
        if (!$pfile->isDot()) {
            $presetname = substr($pfile, 0, strlen($pfile) - 5); // Name - '.scss'.
            if ($filename == $presetname) {
                $scss .= file_get_contents($CFG->dirroot . '/theme/fsgs/scss/preset/' . $pfile);
                $presetisset = true;
            }
        }
    }
    if (!$presetisset) {
        $filename .= '.scss';
        if ($filename && ($presetfile = $fs->get_file($context->id, 'theme_fsgs', 'preset', 0    , '/', $filename))) {
            $scss .= $presetfile->get_content();
        } else {
            // Safety fallback - maybe new installs etc.
            $scss .= file_get_contents($CFG->dirroot . '/theme/fsgs/scss/preset/default.scss');
        }
    }
 ...
 * @return array
 */
function theme_fsgs_get_pre_scss($theme) {
    global $CFG, $PAGE;
 ...
 * @return string
 */
function theme_fsgs_get_extra_scss($theme) {
