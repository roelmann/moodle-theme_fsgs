...
class core_renderer extends \theme_boost\output\core_renderer {
    ...
    public function headerimage() {
        ...
        if (theme_fsgs_get_setting('showcourseheaderimage') && $courseimage) {
        ...
        }elseif (theme_fsgs_get_setting('showcourseheaderimage') && !$courseimage && isset($headerbg)) {
        ...
        }elseif ($courseimage && isset($headerbg) && !theme_fsgs_get_setting('showcourseheaderimage')) {
            $html .= html_writer::start_div('customimage', array(
                'style' => 'background-image: url("'.$headerbgimgurl.'"); background-size: cover; background-position:center;
                width: 100%; height: 100%;'));
            $html .= html_writer::end_div(); // End withoutimage inline style div.
     ...
     */
     public function edit_button(moodle_url $url) {
        ...
        if ($this->page->user_is_editing()) {
            $url->param('edit', 'off');
            $btn = 'btn-danger';
            $title = get_string('editoff' , 'theme_fsgs');
            $icon = 'fa-power-off';
        } else {
            $url->param('edit', 'on');
            $btn = 'btn-success';
            $title = get_string('editon' , 'theme_fsgs');
            $icon = 'fa-edit';
        }
    ...
    protected function render_custom_menu(custom_menu $menu) {
        ...
        if (isloggedin() && !isguestuser() && $hasdisplaymycourses) {
            ...
            if ($mycoursetitle == 'module') {
                $branchtitle = get_string('mymodules', 'theme_fsgs');
            } else if ($mycoursetitle == 'unit') {
                $branchtitle = get_string('myunits', 'theme_fsgs');
            } else if ($mycoursetitle == 'class') {
                $branchtitle = get_string('myclasses', 'theme_fsgs');
            } else {
                $branchtitle = get_string('mycourses', 'theme_fsgs');
            }
            ...
            if ($courses = enrol_get_my_courses(NULL, 'fullname ASC')) {
                ...
                $noenrolments = get_string('noenrolments', 'theme_fsgs');
                $branch->add('<em>'.$noenrolments.'</em>', new moodle_url('/'), $noenrolments);
            }
            if (is_siteadmin()) {
                        $branchtitle = get_string('siteadminquicklink', 'theme_fsgs');
                        $branchlabel = $branchtitle;
    ...
    protected function render_thiscourse_menu(custom_menu $menu) {
        ...
        foreach ($menu->get_children() as $item) {
            $context = $item->export_for_template($this);
            $content .= $this->render_from_template('theme_fsgs/activitygroups', $context);
        }
    ...
    public function thiscourse_menu() {
        ...
        $context = $this->page->context;
            if (isset($COURSE->id) && $COURSE->id > 1) {
                $branchtitle = get_string('thiscourse', 'theme_fsgs');
                $branchlabel = $branchtitle;
                $branchurl = new moodle_url('#');
                $branch = $menu->add($branchlabel, $branchurl, $branchtitle, 10002);

                $data = theme_fsgs_get_course_activities();
    ...
    public function social_icons() {
        ...
        return $this->render_from_template('theme_fsgs/socialicons', $socialcontext);
    ...
    public function fp_wonderbox() {
        ...
        return $this->render_from_template('theme_fsgs/fpwonderbox', $fp_wonderboxcontext);
    ...
    public function fp_marketingtiles() {
        ...
        return $this->render_from_template('theme_fsgs/fpmarkettiles', $fp_marketingtiles);
    }

    public function fp_slideshow() {
        global $PAGE;

        $theme = theme_config::load('fsgs');
        ...
        return $this->render_from_template('theme_fsgs/slideshow', $fp_slideshow);
    }
    ...
    public function teacherdash() {
        ...
        if (isloggedin() && ISSET($COURSE->id) && $COURSE->id > 1) {
            ...
            if (has_capability('moodle/course:viewhiddenactivities', $context)) {
                $togglebutton = get_string('coursemanagementbutton', 'theme_fsgs');
            } else {
                $togglebuttonstudent = get_string('studentdashbutton', 'theme_fsgs');
            }
        }
        $course = $this->page->course;
        $context = context_course::instance($course->id);
        ...
        $userlinks = get_string('userlinks', 'theme_fsgs');
        $userlinksdesc = get_string('userlinks_desc', 'theme_fsgs');
        $qbank = get_string('qbank', 'theme_fsgs');
        $qbankdesc = get_string('qbank_desc', 'theme_fsgs');
        $badges = get_string('badges', 'theme_fsgs');
        $badgesdesc = get_string('badges_desc', 'theme_fsgs');
        $coursemanage = get_string('coursemanage', 'theme_fsgs');
        $coursemanagedesc = get_string('coursemanage_desc', 'theme_fsgs');
        $coursemanagementmessage = (empty($PAGE->theme->settings->coursemanagementtextbox)) ? false : format_text($PAGE->theme->settings->coursemanagementtextbox);
        ...
        //Student Dash
            ...
            $hascourseinfogroup = array (
                'title' => get_string('courseinfo', 'theme_fsgs'),
                'icon' => 'map'
            );
            $summary = theme_fsgs_strip_html_tags($COURSE->summary);
            $summarytrim = theme_fsgs_course_trim_char($summary, 300);
            $courseinfo = array (
                array(
            ...
            $hascoursestaff = array (
                'title' => get_string('coursestaff', 'theme_fsgs'),
                'icon' => 'users'
            );
            ...
            $activitylinkstitle = get_string('activitylinkstitle', 'theme_fsgs');
            $activitylinkstitle_desc = get_string('activitylinkstitle_desc', 'theme_fsgs');
            $mygradestext = get_string('mygradestext', 'theme_fsgs');
            $myprogresstext = get_string('myprogresstext', 'theme_fsgs');
        ...
        if ($globalhaseasyenrollment && $coursehaseasyenrollment) {
            ...
            return $this->render_from_template('theme_fsgs/teacherdash', $dashlinks );
